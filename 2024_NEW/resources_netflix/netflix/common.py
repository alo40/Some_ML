...
def plot_two_plots(X: np.ndarray, mixtures, posts, titles: [str, str]):
    """Plots the mixture model for 2D data"""
    fig, axs = plt.subplots(1, 2)

    for ax, mixture, post, title in zip(axs, mixtures, posts, titles):
        _, K = post.shape
...
def bic(X: np.ndarray, mixture: GaussianMixture, log_likelihood: float) -> float:
    ...
    n, d = X.shape
    K = mixture.p.shape[0]
    p = K * d + K + K - 1  # probabilities sum to 1, so it requires only K-1 parameters
