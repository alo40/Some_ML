...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    for (feature_vector, label) in zip(feature_matrix, labels):
        hinge_sum += hinge_loss_single(feature_vector, label, theta, theta_0)
...
def perceptron_single_step_update(
    ...
    """
    epsilon = np.abs(1e-6) # to avoid numerical instabilities
...
def pegasos_single_step_update(
    ...
    Returns:
        ...
        completed.
    """
    # epsilon = np.abs(1e-9) # to avoid numerical instabilities
    if label * (np.dot(feature_vector, theta) + theta_0) <= 1:
        update_theta_0 = theta_0 + eta * label
        update_theta = theta * (1 - eta * L) + eta * label * feature_vector
    else:
        update_theta_0 = theta_0
        update_theta = theta * (1 - eta * L)
...
def classify(feature_matrix, theta, theta_0):
    ...
    """
    epsilon = np.abs(1e-6)  # to avoid numerical instabilities
    class_vector = np.zeros(feature_matrix.shape[0], )
    for i, feature_vector in enumerate(feature_matrix):
        if np.dot(feature_vector, theta) + theta_0 > epsilon:
            class_vector[i] = 1
        else:
            class_vector[i] = -1
    return class_vector
...
def classifier_accuracy(
    ...
    train_class_vector = classify(train_feature_matrix, train_theta, train_theta_0)
    train_accuracy = accuracy(train_class_vector, train_labels)
    # validation data
    val_class_vector = classify(val_feature_matrix, train_theta, train_theta_0)
    val_accuracy = accuracy(val_class_vector, val_labels)
...
def bag_of_words(texts, remove_stopword=True):
    ...
    if remove_stopword:
        stopwords = load_list_from_file('stopwords.txt')
    else:
        stopwords = ''
