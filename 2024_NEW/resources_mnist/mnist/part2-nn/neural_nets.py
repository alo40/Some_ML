...
def rectified_linear_unit_derivative(x):
    ...
    return np.where(np.array(x) > 0, 1, 0).tolist()
...
class NeuralNetwork():
    ...
    def train(self, x1, x2, y):
        ...
        output = np.dot(self.hidden_to_output_weights, hidden_layer_activation)  # output layer has no bias
        activated_output = output_layer_activation(output)

        ### Backpropagation ###

        # Compute gradients
        cost_function_derivative = -(y - activated_output)
        output_layer_error = cost_function_derivative * output_layer_activation_derivative(output)
        hidden_layer_error = np.multiply(self.hidden_to_output_weights.T * output_layer_error, vec_relu_derivative(hidden_layer_weighted_input))  # (3 by 1 matrix)

        bias_gradients = hidden_layer_error
        hidden_to_output_weight_gradients = hidden_layer_activation * output_layer_error
        input_to_hidden_weight_gradients = np.multiply(input_values.T, hidden_layer_error)
