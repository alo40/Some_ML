...
def plot_decision_line(a: np.float64, b: np.float64, c: np.float64):
    """
...
def plot_theta_norm(theta, x_values, y_values):
    ...
    plt.quiver(0+dx, 0+dy, theta[1], theta[2],
               angles='xy', scale_units='xy', scale=1, color='m', label='normal vector')


def plot_initializing_3d():
    # Create a new figure
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    ...
    # Set title
    ax.set_title('3D Plot of Points [1,1,1] and [2,2,2]')
